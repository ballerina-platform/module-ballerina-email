...
public final class ImapSimpleSecureEmailReceiveTest {
    ...
    public static Object startSimpleSecureImapServer() {
        try {
            startServer();
        } catch (IOException e) {
            return CommonUtil.getBallerinaError(EmailConstants.ERROR,
                    "Failed to start IMAPS server: " + e.getMessage());
        }
        return null;
    }

    public static Object stopSimpleSecureImapServer() {
        mailServer.stop();
        return null;
    }

    public static Object sendEmailSimpleSecureImapServer() {
        try {
            sendEmail();
